<template>
  <v-touch v-on:swiperight="swiperight" class="emoji-wrap">
    <ul class="emoji-list">
		...
		<div class="toast" v-show="adding==1">表情添加中，请稍后...</div>
		<div class="toast" v-show="adding==3">表情添加成功</div>
		<div class="toast" v-show="adding==4">您已添加该表情包</div>
		<div class="emoji-popup" v-if="showPopup" @click="showPopup=0">
			<div :class="['popup-content',showPopup?'in':'out']" @click.stop=";">
				...
				<div class="popup-add" @click="addEmoji(list[showPopup-1].bqbId,showPopup-1)">添加表情</div>
				<div class="popup-cancel" @click="showPopup=0">取消</div>
			</div>
		</div>
  </v-touch>
</template>
...
<script>
	import emoji from '../mixins/myemoji';
	export default {
		name: 'myemoji',
		mixins: [emoji] 
	}
