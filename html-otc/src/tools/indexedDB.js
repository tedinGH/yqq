...
export const EchatDB = {
  ...
  openDB(userId) {
    ...
    return new Promise((resolve, reject) => {
      let indexedDBName = Crypto.encryptByDES(this.name, userId + store.state.cleanNum);
  ...
  dbCheck(e) {
    ...
    if (!storeNames.contains(this.msgPrivateTN)) {
      //单聊消息表创建
      let objectStore = this.db.createObjectStore(this.msgPrivateTN, {
        autoIncrement: true,
      });
      //创建唯一消息索引
      objectStore.createIndex("sessionIdAndMid", ["userId", "mId"], {
        unique: true,
      });
      objectStore.createIndex("userId", "userId", { unique: false });
      objectStore.createIndex("sessionIdAndType", ["userId", "bodyType"], {
        unique: false,
      });
      objectStore.createIndex("sessionIdAndTime", ["userId", "bodyTime"], {
        unique: false,
      }); //用于定期清理
    }
    ...
    if (!storeNames.contains(this.msgServiceTn)) {
      //服务号消息表创建
      let objectStore = this.db.createObjectStore(this.msgServiceTn, {
        autoIncrement: true,
      });
      //创建唯一消息索引
      objectStore.createIndex("sessionIdAndMid", ["userId", "mId"], {
    ...
    if (!storeNames.contains("group_member")) {
      let objectStore = this.db.createObjectStore("group_member", {
        autoIncrement: true,
      });
      //创建唯一群成员索引
    ...
    if (!storeNames.contains(this.friendTN)) {
      let objectStore = this.db.createObjectStore(this.friendTN, {
        keyPath: "userId",
      });
  ...
  insertTempMsg(userId, record) {
    ...
    let stores = this.db
      .transaction("history_temp", "readwrite")
  ...
  readTempMsg(user) {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        if (
          event.target.result &&
          JSON.stringify(event.target.result.record) != "{}"
        ) {
          try {
            ...
            if (Object.keys(this.historyList).length >= 30) {
              ...
              minMsgId =
                user.mId + 1 - Object.keys(this.historyList).length + "";
              time = this.historyList[minMsgId].bodyTime;
            }
            ...
            for (let key in obj) {
              if (time >= key) {
                ...
                for (let k in this.historyList) {
                  if (
                    this.historyList[k].createTime &&
                    this.historyList[k].createTime == key
                  ) {
                    isSend = true;
                    continue;
                  }
