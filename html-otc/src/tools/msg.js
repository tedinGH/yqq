...
const messageParse = {
  ...
  chatListInit(list) {
    ...
    for (let i = 0; i < list.length; i++) {
      let item = list[i];
      //比较MID是否有变动
      let sItem=session[item.fromId+'-'+item.fromType];
      let fault={};//断层获取对象
      fault.sessionId=item.fromId;
      fault.newMId=item.mId;
      fault.sessionType=item.fromType;
      if(sItem){
        if(sItem.mId<item.mId){
          fault.oldMId=sItem.mId;
        }else{
          continue;
        }
      }else{        
      ...
      this.retrieveMsg(fault.sessionId,fault.sessionType,fault.newMId,fault.oldMId);
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (item.fromType == 2 && item.fromId == 1000130) continue;
      //删除聊天记录的数据处理
      if (!item.msg) {
  ...
  updateChat(msgObj) {
    ...
    //消息ID为时间戳 过滤非序列化消息
    if (mId.length > 12 && body.bodyType != 25 && body.bodyType != 31 && body.bodyType != 19&& body.bodyType != 52) return;
 ...
 */
  ...
  updateHistoryStepThree(history,userId,fromType,mId){
      // 缓存新消息到本地
      if (history.bodyType != 31 && history.bodyType != 43 && mId < 10000000000) {
          console.log('消息触发保存')
      ...
      if (userId == store.getters.currentSession.paramId) {
        ...
        if (fromType == "0"&&mId < 10000000000) {
          rxml = `<message xmlns="jabber:client" type="chat" to="${userId}@bcircle.net" id="${time}" from="${store.getters.userId}@bcircle.net/webIm"><body>{"bodyLevel":0, "bodyContent":"{\\\"lastReadId\\\":${mId}}", "bodyType":17, "bodyFrom":"${store.getters.userId}"}</body></message>`;
        }else if (fromType == "1"&&mId < 10000000000) {
          rxml = `<message xmlns="jabber:client" type="chat" to="${userId}@group.bcircle.net" id="${time}" from="${store.getters.userId}@bcircle.net/webIm"><body>{"bodyLevel":0, "bodyContent":"{\\\"lastReadId\\\":${mId}}", "bodyType":17, "bodyFrom":"${store.getters.userId}"}</body></message>`;
        }
        if(rxml){
          if(history.bodyFrom==store.getters.userId) {
  ...
  //标签替换
  replayLabel(obj) {
    obj = obj.replace("<div><br></div>", "<br>").replace(/<br\s*\/?>/gi, "\r\n").replace(/&nbsp;/g, " ").replace(/<div>/g, "\r\n").replace(/<[^>]+>/g, "");
    return obj;
  },
  //替换成纯文本
  replayToText(value) {
 ...
 * @param {*} oldId 旧ID
 */
  retrieveMsg(userId, type, newId,oldId) {
    let differNum = newId - oldId+1;
      //消息丢失
      // 计算断层
      if (differNum > 50) {
        oldId = newId-50;
      }
      msgManager.getFaultMsg((oldId+1)+"-"+newId, userId, type);
  }
};
