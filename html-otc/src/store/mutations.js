...
export default {
  ...
  [types.SET_ISALLOWMOBILE](state, payload) {
    state.isAllowMobile = payload;
  },
  ...
  [types.UPDATE_SESSION](state, payload) {
    ...
    if (cache) {
      if (cache.lastReadId > payload.lastReadId) {
        ...
        payload.unreadNum =
          cache.mId - cache.lastReadId < 0 ? 0 : cache.mId - cache.lastReadId;
      }
    }
    Vue.set(
      state.session.record,
      payload.paramId + "-" + payload.fromType,
      payload
    );
    sessionUtil.setSessionListLocalStore(state.session.record);
  },
  ...
  [types.SET_FORWARD_LIST](state, payload) {
    state.forwardMenu = payload;
  },
  ...
  [types.SET_DELETEGROUPMEMBER](state, payload) {
    state.deleteGroupmember = payload;
  },
  [types.UPDATE_VIDEO_INFO](state, payload) {
    if(payload.obj){
    state.videoInfo = payload.obj
    }else{
  ...
  [types.UPDATE_AUDIO_STATE](state, payload) {
    state.isOnlineAudio = payload;
  },
  ...
  [types.SET_TOAST_BOX](state, payload) {
    state.toastBox = payload;
  },
  ...
  [types.SET_FRIENDCARD](state, payload) {
    state.friendcard = payload;
  },
  ...
  [types.SET_AFFICHE](state, { groupId, body }) {
    if (state.affiche[groupId]) {
      Vue.delete(state.affiche, groupId);
    }
  ...
  [types.SET_TOFIRST](state, payload) {
    state.tofirst = payload;
  },
