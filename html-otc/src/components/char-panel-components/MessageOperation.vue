<template>
  <div class="message-operation" v-if="edit.show || replied.show">
    <div class="msg-box display-flex" v-if="edit.show">
      <!-- <span class="edit-icon"></span> -->
      <div  class="msg-box" v-scrollBar>
        <div  class="reply-text">{{ $t("msg.menu.edit")}}：</div> 
        <div class="display-flex-item"  v-to-face v-text="edit.content.split(' : ')[edit.content.split(' : ').length-1]"></div>
      </div>
    ...
    <div class="reply display-flex" v-if="replied.show">
      ...
      <div class="msg-box " v-scrollBar>
        ...
        <div
          ...
          v-to-face
          v-text="this.replied.history.preview.split(' : ')[this.replied.history.preview.split(' : ').length-1]">
        </div>
        ...
        <div  class="display-flex-item" v-if="replied.history.bodyType == 51">
...
export default {
  ...
  computed: {
    ...mapGetters(["userInfo", "currentSession", "mouseRightInfo",'activityGroupMembers'])
  },
  watch: {
    mouseRightInfo() {
      this.show();
  ...
  methods: {
    show() {
      ...
      if (this.mouseRightInfo.type == "delete") {
        // console.log(this.mouseRightInfo);
        //删除指定会话指定消息
        this.$store.commit("SET_DELETE_INFO", {
          isAdmin:this.mouseRightInfo.isAdmin||this.mouseRightInfo.isMy,
          show: true,
      ...
      if (this.mouseRightInfo.type == "forward") {
        ...
        // delete history.bodyContent.imgUrl
        // history.bodyContent= JSON.stringify(history.bodyContent)
        let forwardInfo = {
          show: true,
          body: [
            {
      ...
      if (this.mouseRightInfo.type == "copy") {
        ...
        if (history.bodyType == 28) {
          content = JSON.parse(history.bodyContent).content;
        }
        if (history.bodyType == 30) {
          content = history.bodyContent.content||JSON.parse(history.bodyContent).content;
        }
        ...
        if(history.bodyType == 24)  content = history.msgContent
      ...
      if(this.mouseRightInfo.type == "forcopy"){
        let history =  this.$store.state.records[this.mouseRightInfo.mId]
        let content = history.bodyContent;
        // console.log(content);
    ...
    editMessage() {
      ...
      content = MessageHandler.textToHtml(content)
      Util.insertAtCursor(this.$parent.$refs.editMsg, content, false);
    },
    sendEdit(text) {
      let cacheMsg;
      ...
      let newMsg = {
        bodyContent: {
          ...
          repliedId: history.msgContent.repliedId
        },
      ...
      cacheMsg=newMsg.bodyContent;
      if(this.currentSession.fromType==1){
        newMsg.bodyLevel=1;
        let key = this.currentSession.paramId + ""
        if(typeof newMsg.bodyContent == "object"){
          newMsg.bodyContent = Crypto.encryptByDES(JSON.stringify(newMsg.bodyContent), key);
        }else{
          newMsg.bodyContent = Crypto.encryptByDES(newMsg.bodyContent, key);
        }
        
      }else if(this.currentSession.fromType==0){
        newMsg.bodyLevel=1;
        let key = this.userInfo.userId + "" + this.currentSession.paramId + ""
        if(typeof newMsg.bodyContent == "object"){
          newMsg.bodyContent = Crypto.encryptByDES(JSON.stringify(newMsg.bodyContent), key);
        }else{
          newMsg.bodyContent = Crypto.encryptByDES(newMsg.bodyContent, key);
        ...
        () => {
          ...
          if (history.bodyType == 28) {
            ...
            if (history.msgContent.editType == 30) {
              ...
              history.bodyContent =JSON.parse(JSON.parse(cacheMsg).content)
            }
          }
    ...
    sendReplied(text) {
      ...
      let bodyContent = {
        ...
        repliedName:this.currentSession.fromType==1 ? (this.activityGroupMembers[history.bodyFrom].nickName||history.chatInfo.nickName): history.chatInfo.nickName,
        repliedObjId:history.bodyFrom
      };
      ...
      if (history.bodyType == 5) {
        bodyContent.repliedContent = history.msgContent.videoUrl;
      }
      ...
      if (history.bodyType == 28) {
        if (history.msgContent.editType == 30) {
          if(typeof history.bodyContent.content == 'string'){
            history.bodyContent = history.bodyContent
          }
      ...
      if(history.bodyType == 51){
        bodyContent.repliedContent =  "[" + this.$t("msg.common.chatRecord") + "]";
      }
      ...
      if ( this.$store.state.friendList[bodyContent.repliedObjId] ) {
      if(this.$store.state.friendList[bodyContent.repliedObjId].notes){
        bodyContent.repliedName=this.$store.state.friendList[bodyContent.repliedObjId].notes
      }
    }
