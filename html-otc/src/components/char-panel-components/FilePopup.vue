    ...
    methods: {
      ...
      initFile(file, i) {
        ...
        if(!this.filesize){
          ...
          overSize = this.filesize
        }
        if (file.size == 0 || file.size > 1024 * 1024 * overSize) {
          this.$store.commit("SET_TOAST_TEXT", this.$t("msg.chatPanel.overSize"));
          return false;
        }
      ...
      uploadFile(fileBlock, uploadId) {
        return new Promise((resolve, reject) => {
          ...
          let that=this;
      reader.readAsArrayBuffer(fileBlock.file);
      reader.onload = function(){
        var data = this.result;
              ...
              try {
                ...
                let myArray=msgUtil.msgEntityToBufferV1(that.$store.getters.token,"/fileShare/fileUploadV2.htm",dataParams,data);
      ...
      uploadFile1(fileBlock, uploadId) {
        return new Promise((resolve, reject) => {
      ...
      uploadHandler(cacheFile, uploadCount) {
        ...
        for(let key in this.fileList){
          let b=this.fileList[key]
          if(b.fileId==cacheFile.fileId){
            cancel=false;
            break;
          }
        }
