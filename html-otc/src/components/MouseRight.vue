<template>
  <transition name="drop-left">
    <div class="mouse-right" :style="{ top: mouseMenu.top + 'px', left: mouseMenu.left + 'px' }"
      v-if="layout.child == 'mr'">
      <ul @click.stop="" unselectable="on">
        <!-- 消息右键 -->
        <li class="mouse-right-item copy" @click="doMessage('copy')" v-if="mouseMenu.copy">
          <span :title="$t('msg.menu.copy')">{{ $t("msg.menu.copy") }}</span>
        </li>
         <li class="mouse-right-item copy" @click="doMessage('forcopy')" v-if="mouseMenu.forcopy">
          <span :title="$t('msg.menu.copy')">{{ $t("msg.menu.copy") }}</span>
        </li>
        <li class="mouse-right-item forward" @click="doMessage('forward')" v-if="mouseMenu.forward">
          <span class="forward" :title="$t('msg.menu.forward')">{{
            $t("msg.menu.forward")
          }}</span>
        </li>
        <!-- <li
        ...
        <li class="mouse-right-item delete" @click="doMessage('delete')" v-if="mouseMenu.delete">
          <span :title="$t('msg.menu.delete')">{{
            $t("msg.menu.delete")
          }}</span>
        </li>
        <li class="mouse-right-item delete" @click="doMessage('emojiDelete')" v-if="mouseMenu.emojiDelete">
          <span :title="$t('msg.menu.delete')">{{
            $t("msg.menu.delete")
          }}</span>
        </li>
        <li class="mouse-right-item reply" @click="doMessage('reply')" v-if="mouseMenu.reply">
          <span :title="$t('msg.menu.reply')">{{ $t("msg.menu.reply") }}</span>
        </li>
        <li class="mouse-right-item edit" @click="doMessage('edit')" v-if="mouseMenu.edit">
          <span :title="$t('msg.menu.edit')">{{ $t("msg.menu.edit") }}</span>
        </li>
        <li class="mouse-right-item isMute" @click="videoPlay" v-if="mouseMenu.isMute">
        ...
        <li class="mouse-right-item copy" @click="selectCopy" v-if="mouseMenu.selectCopy">
          <span :title="$t('msg.menu.copy')">{{ $t("msg.menu.copy") }}</span>
        </li>
        <li class="mouse-right-item more" @click="doMessage('more')" v-if="mouseMenu.more">
          <span :title="$t('msg.menu.more')">{{ $t("msg.menu.more") }}</span>
        </li>
        <!-- 会话右键 -->
        <li class="mouse-right-item isTop" @click="doSession('isTop')" v-if="mouseMenu.isTop">
          <span :title="
        ...
        <li class="mouse-right-item isInterruption" @click="doSession('isInterruption')"
        ...
        <li class="mouse-right-item delete" @click="doSession('isDelete')" v-if="mouseMenu.isDelete">
          <span :title="$t('msg.menu.deleteChat')">{{
            $t("msg.menu.deleteChat")
          }}</span>
        </li>
        <!-- 搜索消息右键 -->
        <li class="mouse-right-item forward" @click="doRecord('forward')" v-if="mouseMenu.recordForward">
          <span :title="$t('msg.menu.forward')">{{
            $t("msg.menu.forward")
          }}</span>
        </li>
        <li class="mouse-right-item copy" @click="doRecord('copy')" v-if="mouseMenu.recordCopy">
          <span :title="$t('msg.menu.copy')">{{ $t("msg.menu.copy") }}</span>
        </li>
        <li class="mouse-right-item more" @click="doRecord('more')" v-if="mouseMenu.recordMore">
          <span :title="$t('msg.menu.more')">{{ $t("msg.menu.more") }}</span>
        </li>
        <!-- <li
          class="mouse-right-item favorites"
          @click="doRecord('favorites')"
          v-if="mouseMenu.recordFavorites"
        >
          <span :title="$t('msg.menu.favorite')">{{ $t("msg.menu.favorite") }}</span>
        </li> -->
        <li class="mouse-right-item goMsg" @click="doRecord('goMsg')" v-if="mouseMenu.recordGoMsg">
        ...
        <!-- <li class="mouse-right-item temporary" @click="tempSession" v-if="mouseMenu.tempSession">
          <span :title="$t('msg.menu.tempSession')">{{
            $t("msg.menu.tempSession")
          }}</span>
        </li> -->
        <li class="mouse-right-item toa" @click="sendAtMsg" v-if="mouseMenu.atMember">
          <span :title="'@' + memberInfo.nickName">@{{memberInfo.notes|| memberInfo.nickName }}</span>
        </li>
        <li class="mouse-right-item banned" @click="forbiddenOne" v-if="mouseMenu.isBanned">
          <span :title="
        ...
        <!-- <li class="mouse-right-item doShield" @click="doShield" v-if="mouseMenu.shield">
          <span :title="
        ...
        <li class="mouse-right-item manager" @click="setGroupAdmin" v-if="mouseMenu.setAdmin">
          <span :title="
        ...
        <li class="mouse-right-item transferGroup" @click="transferGroup" v-if="mouseMenu.transferGroup">
          <span :title="$t('msg.menu.transferGroup')">{{
            $t("msg.menu.transferGroup")
          }}</span>
        </li>
        <li class="mouse-right-item delete" @click="deleteGroupMember"
          v-if="mouseMenu.deleteMember && (memberInfo.isAdmin == 0||memberInfo.isAdmin == 2)">
...
import { friend } from "@/session/friend";
  export default {
    ...
    computed: {
      ...mapGetters(["userId", "userInfo", "currentSession", "activityGroupMembers", "layout",'records']),
      messageList() {
    ...
    methods: {
      menuFilters(data) {
        ...
        switch (data.type) {
          case "message":
            ...
            //语音
            if (this.message.bodyType == 4||this.message.bodyType == 29) {
            ...
            break;
        case "emoji":
          this.mouseMenu.emojiDelete = true;
        break;
        case "forword":
          this.mouseMenu.forcopy = true;
        break;
          case "session":
      ...
      doMessage(type) {
        ...
        if(type=='forcopy'){
          mouseRightInfo.mId = this.mrData.mid
        }
        if(type=='emojiDelete'){
          // console.log(this.$parent.$refs.myemoji);
          // this.$parent.$refs.myemoji.delname = this.mrData.name
          this.$parent.$refs.myemoji.delid = this.mrData.mid
          this.$parent.$refs.myemoji.emojidel = true
        }
        // debugger
      ...
      deleteGroupMember() {
        // this.$parent.$refs.chat.$refs.chatHistory.$refs.groupInformation.showConfirm = true
        // this.$parent.$refs.chat.$refs.chatHistory.$refs.groupInformation.delid = this.memberInfo.userId
        let obj = {
          show:true,
          id:this.memberInfo.userId
        }
        this.$store.commit("SET_DELETEGROUPMEMBER", obj);
        if(this.$parent.$refs.chat.$refs.chatHistory.$refs.groupInformation){
      ...
      selectCopy() {
        let oInput = document.createElement("input");
        oInput.value = MessageHandler.htmlToText(this.selectedText);
        document.body.appendChild(oInput);
        oInput.select(); // 选择对象
        document.execCommand("Copy");
        oInput.style.display = "none";
      ...
      onMouseRight() {
        ...
        if (this.mrData.type == "forword") {
        if(this.mrData.msgType!==1&&this.mrData.msgType!==24&&this.mrData.msgType!==13&&this.mrData.msgType!==28&&this.mrData.msgType!=30){
          this.$store.dispatch("setLayout", ["", "", false]);
        }
        }
...
<style lang="scss">
  ...
  .mouse-right {
    ...
    .mouse-right-item {
      ...
      &.copy {
        background: url(../assets/images/icon/copy.png) no-repeat;
      ...
      &.forward {
        background: url(../assets/images/icon/forward.png) no-repeat;
        background-position: 14px;
      ...
      &.delete {
        background: url(../assets/images/icon/delete2.png) no-repeat;
        background-position: 14px;
      ...
      &.reply {
        background: url(../assets/images/icon/reply2.png) no-repeat;
        background-position: 14px;
      ...
      &.edit {
        background: url(../assets/images/icon/edit2.png) no-repeat;
        background-position: 14px;
      ...
      &.isTop {
        background: url(../assets/images/icon/isTop.png) no-repeat;
        background-position: 14px;
      ...
      &.isInterruption {
        background: url(../assets/images/icon/isInterruption.png) no-repeat;
        background-position: 14px;
      ...
      &.goMsg {
        background: url(../assets/images/icon/goMsg.png) no-repeat;
        background-position: 14px;
      ...
      &.banned {
        background: url(../assets/images/icon/banned.png) no-repeat;
        background-position: 14px;
      ...
      &.manager {
        background: url(../assets/images/icon/manager.png) no-repeat;
        background-position: 14px;
      ...
      &.toa {
        background: url(../assets/images/icon/toa.png) no-repeat;
        background-position: 14px;
      ...
      &.transferGroup {
        background: url(../assets/images/icon/transferGroup.png) no-repeat;
        background-position: 14px;
      ...
      &.temporary {
        background: url(../assets/images/icon/temporary.png) no-repeat;
        background-position: 14px;
      ...
      &.doShield {
        background: url(../assets/images/icon/doShield.png) no-repeat;
        background-position: 14px;
      ...
      &.isMute {
        background: url(../assets/images/icon/isMute.png) no-repeat;
        background-position: 14px;
