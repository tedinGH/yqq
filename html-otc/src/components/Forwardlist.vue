<template>
  <!-- 转发 -->
  <div class="my-popup forward" @click="forwardMenu.show = false">
    <div class="my-popup-content display-flex" @click.stop=";">
      <span class="my-popup-close" @click.stop="forwardMenu.show = false"></span>
      <div class="right-content">
        ...
        <ul class="forward-list" v-scrollBar>
          <li class="forward-list-item display-flex" v-for="(item, idx) in selectlist" :key="idx"
            ...
            <div class="item-left">
              ...
              <!-- <span class="default" v-defaultHead v-if="item.fromType == 1 && !item.img">{{ item.name }}</span> -->
            </div>
        ...
        <button class="two-btn cannel" @click="forwardMenu.show = false">
          {{ $t("msg.common.cancel") }}
        </button>
      </div>
      <div class="left-content">
            ...
            <li class="forward-list-item display-flex" :class="{ hide: item.temp || item.fromType == 2 }"
              ...
              <div class="item-left">
                ...
                <!-- <span class="default" v-defaultHead
                  v-if="(item.fromType == 0 || item.fromType == 1) && !item.img">{{ item.name }}</span> -->
              </div>
              <div class="item-right display-flex-item">
                <span class="name-line" v-text="item.name"></span>
              </div>
        ...
        <ul class="forward-list" v-scrollBar v-show="searchKey != ''">
          ...
          <div class="display-flex display-flex-item" style="flex-direction: column "
            v-if="Object.keys(forwardResult).length ==0">
    ...
    methods: {
      ...
      doForward() {
        ...
        if(this.flag){
            ...
            for (var j = 0; j < this.forwardMenu.body.length; j++) {
              let item = this.forwardMenu.body[j]
              let content = {
            ...
            setTimeout(() => {
            this.$store.commit("SET_FORWARD_LIST", {
              show: false,
              body: [],
            });
            this.$parent.$refs.chat.$refs.chatHistory.doShowSuccess2();
            }, 500);
          this.sendOne(arr, list)
      ...
      checkedmessage(item) {
        // debugger
        if (item.bodyType == 1 || item.bodyType == 13 || item.bodyType == 24 ) {
          return item.bodyContent
        }
        // if(item.bodyType ==24) {
        //   let bodyContent = JSON.parse(item.bodyContent);
        ...
        //         showText[i + 1];
        //     } else {
        //       let userInfo = this.activityGroupMembers[bodyContent.userIds[i]];
        //       inviter = inviter + "@" + userInfo.nickName + " " + showText[i + 1];
        //     }
        //   }
        //   return inviter
        // }
        if(item.bodyType == 28) return JSON.parse(item.bodyContent).content
        if(item.bodyType == 30) return  item.bodyContent.content || JSON.parse(item.bodyContent).content
        if (item.bodyType == 2) return ''
        if (item.bodyType == 3) return ''
        if (item.bodyType == 5) return  ''
        if (item.bodyType == 18) return ''
      },
      sendOne(msg, list) {
        let arr = [] 
        for (var j = 0; j < this.forwardMenu.body.length; j++) {
          ...
          let content = {
            nickName: item.nickName,
            message: this.checkedmessage(item),
            msgType: item.bodyType,
          }
       
          arr.push(content)
        }
        this.$http.addCopyChatMessage({
          list: JSON.stringify(msg)
        }).then(
          res => {
            return new Promise((resolve, reject) => {
              for (var i = 0; i < list.length; i++) {
                ...
                let obj = {
                  historyId: res,
                  historyCount: this.forwardMenu.body.length,
                  chatUsers: this.forwardMenu.body[0].chatUsers,
                  contents: arr,
                  chatType: (this.forwardMenu.body[0].chatUsers).length == 1 ? 1 : 0
                };
                let objs = {
                msg:obj,
                msgType: 51,
                ...
                if (chat) {
                  ...
                  chat.mId = chat.mId + 1;
                  chat.msgType = 51;
                  chat.preview = "["+this.$t('msg.common.chatRecord')+"]",
                  chat.userTime = time;
                  chat.fromType = item.fromType
                  this.$store.dispatch("sendMsg", objs);
                  this.upnew(obj, time)
                  ...
                  sessionInfo.getChatInfo(item.fromType, item.paramId).then((chatInfo) => {
                    var newChat = {
                      ...
                      msgType: 51,
                      preview: "["+this.$t('msg.common.chatRecord')+"]",
