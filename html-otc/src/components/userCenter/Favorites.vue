  ...
  <div class="display-flex fa" v-show="value" @click.stop="menuData.show = false">
    ...
    <div class="fa-message">
      ...
      <div class="message-more display-flex" v-if="showMore">
        ...
        <div class="delete-more" @click="showDelete" :class="{ disable: deleteStatues }">
      ...
      <ul class="message-list" @ps-y-reach-end="loadMore" v-scrollBar>
        ...
        <li v-for="(item, index) in chatHistory" :key="index"
          ...
          <div class="message-item" v-if="
              ...
              item.bodyType == '30'
            ">
            <div class="display-flex menu" @click.stop="clickMenu($event, item)">
              ···
            </div>
            <div class="item-left">
            ...
            <div class="item-right">
              ...
              <div class="content text" @click="openDetail(item)" :title="item.msgContent" v-to-face
                v-text="item.msgContent"></div>
            </div>
          </div>
          <!-- 表情消息 -->
          <div class="message-item" v-if="item.bodyType == '2'">
            ...
            <div class="item-right">
              ...
              <div class="content">
                <img class="img" :src="
                    global.fileDownUrl + 'compress/' + item.msgContent.bqUrl
                  " />
              </div>
            </div>
          </div>
          ...
          <div class="message-item" v-if="item.bodyType == '3'">
            ...
            <div class="item-right">
              ...
              <div class="content">
                <img v-imageError class="img" :data-idx="index" :src="
                    global.fileDownUrl + 'compress/' + item.msgContent.imgUrl
                    ...
                    <div class="voice-right">
                      <div class="voice-progress" @click="setCurrentTime($event, item, index)">
                        <span class="open-progress" :style="{
                            width: pxNum(
                              item.msgContent.proNum,
                              item.msgContent.duration
                            ),
                          }"></span>
                        <span class="unread-progress" v-if="
                            !item.msgContent.isPlay && userId != item.bodyFrom
                          "></span>
                      </div>
                      <div class="voice-detail">
                        <span v-if="item.msgContent.proNum != 0">00:<span
                            v-text="timeNum(item.msgContent.proNum)"></span>/</span><span>00:{{
                            item.msgContent.duration > 9
                              ? item.msgContent.duration
                              : "0" + item.msgContent.duration
                          }}</span>
                        <span class="time">{{
                          item.bodyTime | timeFilter
                        }}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div class="item-right">
              ...
              <div class="content">
                <div class="video" v-videoError>
                  <img class="img" :src="
                      global.fileDownUrl + 'compress/' + item.msgContent.imgUrl
                    " :data-idx="index" :mId="item.id" v-faViewer="chatHistory" />
                </div>
              </div>
            </div>
          </div>
              ...
              <div class="content">
                <div class="file">
                  <div class="file-info">
                    ...
                    <div class="size">
                      <span>{{ item.msgContent.size | fileSize }}</span>
                    </div>
                    <div class="download">
                      <a target="_blank" :alt="item.msgContent.name" :href="
                          global.fileDownUrl +
                          'html/download1.html#' +
                          item.msgContent.dl64
              ...
              <div class="content">
                <div class="card" @click="clickCard($event, item)">
                  ...
                  <div class="person-content">
                    <img v-headError :src="
                        global.fileDownUrl + 'compress/' + item.chatInfo.avatar
                      " class="person-avatar" />
    ...
    <transition name="pop">
      <div class="menu-box" :style="{ top: menuData.top + 'px', left: menuData.left + 'px' }" v-show="menuData.show"
        @click.stop="">
        <ul class="menu-list">
          <li class="menu-item" :title="$t('msg.menu.copy')" v-if="getCopyStatus(menuData.item)" @click="
              menuData.show = false;
              doCopy(menuData.item);
            ">
            {{ $t("msg.menu.copy") }}
          </li>
          <li class="menu-item" :title="$t('msg.menu.forward')" v-if="getForwardStatus(menuData.item)" @click="
              menuData.show = false;
              doForward(menuData.item);
            ">
            {{ $t("msg.menu.forward") }}
          </li>
          <li class="menu-item" :title="$t('msg.menu.more')" @click="
              menuData.show = false;
              showMore = true;
            ">
            {{ $t("msg.menu.more") }}
          </li>
          <li class="menu-item" :title="$t('msg.menu.delete')" @click="
              menuData.show = false;
              showConfirm = true;
            ">
            {{ $t("msg.menu.delete") }}
      ...
      <div class="my-popup" v-show="showDetail" @click.stop="">
        <div class="my-popup-content detail">
          ...
          <div class="detail-tip">
            <button class="forward" :title="$t('msg.menu.forward')" @click="doForward(detail)">
              {{ $t("msg.menu.forward") }}
            </button>
            <button class="del" :title="$t('msg.menu.delete')" @click="
                menuData.item = detail;
                showDetail = false;
                showConfirm = true;
              ">
              {{ $t("msg.menu.delete") }}
    ...
    <transition name="scale-x">
      <div class="my-popup" v-show="showNotes" @click.stop="">
        <div class="my-popup-content notes">
          ...
          <tinymce-editor ref="editor" v-if="showNotes" v-model="editorHtml">
          </tinymce-editor>
...
import TinymceEditor from "@/components/tinymce-editor";
  export default {
    ...
    methods: {
      ...
      editSave() {
        if (!this.editorHtml) {
          this.$store.commit("SET_TOAST_TEXT", this.$t("msg.note.none"));
          return false;
        }
        var faObj = {
      ...
      search(type) {
        ...
        if (this.send.searchKey != "")
          obj["params[collectionHead]"] = this.send.searchKey;
