 ...
 */
   ...
   */
  getFaultMsg(msgStr,fromId,fromType){
    let obj = {
  ...
  getFaultMsgHttp(obj){
    ...
    api.getOrders(obj).then(data => {
      if(data!=undefined&&data.length>0){
        if(fromType==10){
          return this.instructionExecution(data);
        }
      ...
      if (body.bodyType == 28) {
        ...
        if(body.bodyContent.type!=-1){
          ...
          for(let key in idArray){
            this.removeMsg(body.bodyFromType,body.bodyFrom,idArray[key]);
          }
        }else{
          this.removeMsgAll(body.bodyFromType,body.bodyFrom);
        }            
      }
    }
  },
