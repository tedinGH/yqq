...
export const msgFormatTemplate = {
            ...
            if (fromType == 1 && paramId != data.bodyFrom && !data.noChatInfo) {
              group.getMemberInfo(paramId, data.bodyFrom).then((member) => {
                ...
                if (data.bodyType == 47) data.preview = data.preview;
                  ...
                  if (store.state.friendList[data.chatInfo.userId]) {
                    if (store.state.friendList[data.chatInfo.userId].notes) {
                      data.chatInfo.nickName =
                        store.state.friendList[data.chatInfo.userId].notes;
                    }
                  }
                  data.preview = data.chatInfo.nickName + " : " + data.preview;
                }
                resolve(data);
              });
            } else {
      ...
      try {
        ...
        switch (bodyType) {
          ...
          case 32: //名片分享  shareCardTemp
            ...
            break;
          case 33: //投诉 complaint
            this.complaint(resolve, content);
            break;
          ...
          case 45: //系统公告协议  systemNoticTemp
            ...
            break;
          case 47: //公告  afficheTemp
            this.afficheTemp(resolve, content);
            break;
          case 51: //聊天记录
            this.msgForwardingTemp(resolve, content);
            break;
          case 52: //好友通讯录加入协议  contactDirTemp
            this.contactDirTemp(resolve, content);
            break;
          case 53: // 验证CaptchaProtocol
            this.CaptchaProtocol(resolve, content);
            break;
        ...
        content.preview =
          error == "Not currently supported"
            ? lang.msgPanel.notSupported
            : lang.msgPanel.parseError;
        content.msgContent =
          error == "Not currently supported"
            ? lang.msgPanel.notSupported
            : lang.msgPanel.parseError;
  ...
  offlineAudioTemp(resolve, content) {
    ...
    /**预下载 */
    // let imgUrl = Config.fileDownUrl + "original/" + content.msgContent.voiceUrl;
    // this.preDownload(imgUrl);
    resolve(content);
  },
  /**
  ...
  readPacketTemp(resolve, content) {
    content.msgContent =
      store.getters.userId == content.bodyFrom
        ? lang.msgPanel.sendRedPacket
        : lang.msgPanel.receiveRedPacket;
    ...
    if (store.getters.userId == content.msgContent.fromId) {
      content.msgContent =
        content.msgContent.status == "1"
          ? lang.msgPanel.otherConfirm
          : lang.msgPanel.sendTransfer;
    } else {
      content.msgContent =
        content.msgContent.status == "1"
          ? lang.msgPanel.confirmTransfer
          : lang.msgPanel.receiveTransfer;
    }
  ...
  groupInstructionTemp(resolve, content) {
    ...
    if (msgBody.type == 2) {
      ...
      if (store.state.activityGroupMembers[msgBody.inviter[0].userId]) {
        msgBody.inviter[0].userName =
          store.state.activityGroupMembers[msgBody.inviter[0].userId].notes ||
          store.state.activityGroupMembers[msgBody.inviter[0].userId].nickName;
      } else {
        if (store.state.friendList[msgBody.inviter[0].userId]) {
          if (store.state.friendList[msgBody.inviter[0].userId].notes) {
            msgBody.inviter[0].userName =
              store.state.friendList[msgBody.inviter[0].userId].notes;
          }
        }
      }
      content.preview =
      ...
      content.preview =
        (msgBody.operatorId != store.getters.userId
          ? msgBody.operatorName
          : lang.common.you) +
        (msgBody.auditResult == 1 ? lang.common.accept : lang.common.refuse) +
        (msgBody.inviter[0].userId != store.getters.userId
          ? msgBody.inviter[0].userName
          : lang.common.you) +
    ...
    } else if (msgBody.type == 35) {
      // msgBody.auditResult:0 关闭，1 开启
      content.msgContent = lang.msgPanel.notSupported;
      content.preview = lang.msgPanel.notSupported;
    }
    content.noChatInfo = true;
    // console.log(content);
    resolve(content);
    ...
    if (userIds.length == 1) {
      if (userIds[0] == "allMember") {
        ...
        sessionInfo.getChatInfo(1, fromId, userIds[0]).then((data) => {
          if (store.state.friendList[userIds[0]]) {
            if (store.state.friendList[userIds[0]].notes) {
              data.nickName = store.state.friendList[userIds[0]].notes;
            }
          }
          // debugger
          showText = showText + "@" + data.nickName + " " + atText[1];
          content.msgContent = this.xssHandler(showText);
          content.preview = this.xssHandler(showText, true, true);
          resolve(content);
        });
    ...
    if (content.bodyContent == 2) {
      content.preview =
        content.bodyFrom == store.getters.userId
          ? lang.msgPanel.isFriend
          : lang.msgPanel.isPass;
  ...
  msgEditTemp(resolve, content) {
    ...
    if (content.msgContent.editType == 30) {
      if (!(typeof content.msgContent.content == "object")) {
        content.msgContent.content = JSON.parse(content.msgContent.content);
      }
      let editBody = content.msgContent.content;

      content.preview = editBody.content;
      content.msgContent.content.content = this.xssHandler(editBody.content);
      ...
      content.msgContent.content = this.xssHandler(content.msgContent.content);
    }
    // content.noChatInfo=true;
    resolve(content);
  },
  ...
  msgReplyTemp(resolve, content) {
    content.msgContent =
      typeof content.bodyContent == "object"
        ...
        : JSON.parse(content.bodyContent);
    content.preview = this.xssHandler(content.msgContent.content, true, true);
    if (store.state.friendList[content.msgContent.repliedObjId]) {
      if (store.state.friendList[content.msgContent.repliedObjId].notes) {
        content.msgContent.repliedName =
          store.state.friendList[content.msgContent.repliedObjId].notes;
      }
    }
    ...
    switch (content.bodyContent.state) {
      ...
      case "5":
        content.msgContent =
          (isMe
            ? lang.common.cancel + " "
            : lang.msgPanel.other + lang.msgPanel.cancel) +
          (content.bodyContent.vType == "1"
            ? lang.msgPanel.video
            : lang.msgPanel.voice);
        break;
  ...
  forbiddenTemp(resolve, paramId, content) {
    ...
    group.getMemberInfo(paramId, store.getters.userId).then((member) => {
      ...
      if (admin > 0) {
        if (store.state.activityGroupMembers[banedBody.bannedPersonId]) {
          banedBody.bannedPersonName =
            store.state.activityGroupMembers[banedBody.bannedPersonId].nickName;
        }
        if (store.state.friendList[banedBody.bannedPersonId]) {
          if (store.state.friendList[banedBody.bannedPersonId].notes) {
            banedBody.bannedPersonName =
              store.state.friendList[banedBody.bannedPersonId].notes;
          }
        }
        if (banedBody.type == 1) {
          if (_langIdx == 0) {
            if (banedBody.state == 1)
              content.preview = `${banedBody.bannedPersonName} 被 ${
                banedBody.operatorId != store.getters.userId
                  ? banedBody.operatorName
                  : lang.common.you
              }设置了禁言`;
            else
              content.preview = `${
                banedBody.operatorId != store.getters.userId
                  ? banedBody.operatorName
                  : lang.common.you
              } 取消了 ${banedBody.bannedPersonName}的禁言`;
          } else if (_langIdx == 1) {
            if (banedBody.state == 1)
              content.preview = `${banedBody.bannedPersonName} Muted by ${
                banedBody.operatorId != store.getters.userId
                  ? banedBody.operatorName
                  : lang.common.you
              }`;
            else
              content.preview = `${banedBody.bannedPersonName} Unmuted by ${
                banedBody.operatorId != store.getters.userId
                  ? banedBody.operatorName
                  : lang.common.you
              }`;
        ...
        if (banedBody.type == 1) {
          if (_langIdx == 0) {
            if (banedBody.state == 1)
              content.preview = `${
                banedBody.bannedPersonId != store.getters.userId
                  ? banedBody.bannedPersonName
                  : lang.common.you
              } 被管理员设置了禁言`;
            else
              content.preview = `管理员取消了${
                banedBody.bannedPersonId != store.getters.userId
                  ? banedBody.bannedPersonName
                  : lang.common.you
              }的禁言`;
   ...
   */
  // 投诉
  complaint(resolve, content) {
    let item = content.bodyContent.type;
    if (item == 0) content.preview = lang.complaint.submitted;
    if (item == -1) content.preview = lang.complaint.unsuccess;
    if (item == 1) content.preview = lang.complaint.success;
    if (item == 2) content.preview = lang.complaint.Warningnotice;
    if (item == 3) content.preview = lang.complaint.Violationof;
    if (item == 4) content.preview = lang.complaint.Bannednotice;
    if (item == 5) content.preview = lang.complaint.Accountnotice;
    resolve(content);
  },
  //验证
  CaptchaProtocol(resolve, content) {
    if (content.bodyContent.type == 10004)
      content.preview = lang.complaint.Loginverification;
    else content.preview = lang.complaint.Accountsecurity;
    resolve(content);
  },
  // 安全通知
  SafetyNoticeProtocol(resolve, content) {
    let item = content.bodyContent.type;
    if (item == 10010) content.preview = lang.complaint.Psdchange;
    if (item == 10017 || item == 17)
      content.preview = lang.complaint.Accountsuccess;
    if (item == 10018 || item == 18)
      content.preview = lang.complaint.Accountunsuccess;
    if (item == 10006) content.preview = lang.complaint.phonechanged;
    resolve(content);
  },
  // 客服
  ...
  afficheTemp(resolve, content) {
    content.msgContent = JSON.parse(content.bodyContent);
    content.msgContent.affiche = this.xssHandler(
      content.msgContent.affiche,
      false,
      false
    );
  ...
  contactDirTemp(resolve, content) {
    let msgContent = content.bodyContent;
    content.preview = msgContent.nickName + lang.msgPanel.joined + "EchatAPP";
    content.msgContent = content.preview;
    resolve(content);
  },
  /**预下载 图片 */
  preDownload(url){
    ...
    //   win.postMessage(url,'*');//向不同域的www.script.com/b.html发送消息
