<template>
	<div class="code" v-if="isShow">
		<div class="code-content">
			<div class="code-head"><span>{{$t('msg.register.clickBlow')}}</span><span class="code-col" v-text="codeStr"></span></div>
			<div class="code-bg">
				<img src="../../assets/img/reg-back.png" />
				<div class="codebox">
			...
			<div class="code-wait en" :class="{active: codeTimeStatus}" >{{$t('msg.register.busy')}}<span v-text="codeTime"></span>s</div>
			<div class="code-refash" @click="doRefresh()"></div>
			<div class="code-cancel" @click="doCancel()"></div>
			<a href="javascript:;" class="code-btn" :class="{active: comfirmStatus}" @click="doConfirm()">{{$t('msg.register.confirm')}}</a>
		</div>
	</div>
</template>
...
import { Util } from '@/tools/utils';
	...
	export default {
		...
		methods: {
			...
			//获取验证码
			getCodeInfo() {
			this.$api.createSelectCode({}).then(
					data => {
            // console.log(data.data.selectionCodeKey);
						this.data = data.data
            this.codeData = this.codeToArr(this.data);
          ...
          result = [];
				for(let key in codeObj.codeMap){
					let value = Util.decryptByDES(codeObj.codeMap[key], codeObj.selectionCodeKey),
        ...
        this.$api.checkSelectCode(postData).then(
					res => {
            if(res.code==0){
              this.$store.commit('SET_SELECTIONCODEKEY', res.data);
...
<style scoped>
	...
	.code-refash {
		...
		height: 34px;
		width: 30px;
		background-image: url(../../assets/img/refash.png);
