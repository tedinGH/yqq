...
import { officialDB } from "./officialDB";
...
export const official = {
  ...
  getMsg(sessionId, maxMsgId, firstMsgId, pageSize = 20) {
    ...
    return new Promise(resolve => {
      ...
      officialDB.readPrivateMsgPage(sessionId, maxMsgId, pageSize).then(data => {
        data.sort((x, y) => {
          return x.id - y.id;
        });
        if (data.length == 0) {
          if (maxMsgId > 0) {
            return this.getNetworkMsgList(maxMsgId, 0, sessionId, data, resolve);
          }
        } else {
          data.sort((x, y) => {
            return x.id - y.id;
          ...
          if (cacheMsg.id < maxMsgId) {
            return this.getNetworkMsgList(maxMsgId, cacheMsg.id + 1, sessionId, data, resolve);
          } else {
            if (data.length < 30 && data[0].id > 1) {
              return this.getNetworkMsgList(data[0].id - 1, 0, sessionId, data, resolve, true);
            }
          }
        }
        return resolve(data);
  ...
  updateMsg(data) {
    //更新数据库
    officialDB.updatePrivateMsg(data);
  },
  updateMsgBatch(sessionId, data) {
    //更新数据库
    sessionId = Number(sessionId);
    officialDB.updatePrivateMsgBatch(sessionId, data);
  },
  ...
  getNetworkSession(userId) {
    ...
    return new Promise(resolve => {
      actionApi.getTempFriendInfo({ sessionId: userId }).then(data => {
        ...
        officialDB.updateFriend(userId, data);
        //更新当前session
  ...
  removeMsgAll(sessionId) {
    officialDB.removeMsgAll(sessionId);
  },
  /**
   * 移除指定消息
   */
  removeMsg(sessionId, mId, isAll) {
    officialDB.removeMsg(sessionId, mId, isAll);
  },
  /**
   * 清空所有缓存数据
   */
  ...
  regularCleaning(sessionId, lastTime, cycle) {
    officialDB.regularCleaning(sessionId, lastTime, cycle);
  },
  editMsg(sessionId, mId, content) {
    officialDB.editMsg(sessionId, mId, content);
  },
  setReadMsgBySessionId(sessionId, maxReadId) {
    officialDB.setReadMsgBySessionId(sessionId, maxReadId);
  },
  ...
  getLocalMsg(sessionId, mId) {
    return new Promise(resolve => {
      officialDB.getLocalMsg(resolve, sessionId, mId);
    });
  },
  /**
   * 获取本地消息
  ...
  getLocalImgAndVideo(sessionId) {
    return new Promise(resolve => {
      officialDB.getLocalImgAndVideo(resolve, sessionId);
    });
  },
  vagueFind(keyWord) {
    return new Promise(resolve => {
      officialDB.vagueFind(resolve, keyWord);
    });
  },
  /**
   * 更新消息已读
  ...
  updateMsgRead(userId, mId) {
    officialDB.updateMsgRead(userId, mId);
  }
};
