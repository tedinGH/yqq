      ...
      <div class="content">
        <ul class="list" v-scrollBar>
          ...
          <li v-for="(item,idx) in question" :key="idx" class="question">
            <span v-if="item.isRequired==1" style="color:#ff5959;margin-right:8px">*</span><span class="title">{{item.title}}</span>
            ...
            <el-date-picker class="time" style="display:block;" v-if="item.elementType==4" v-model="item.models" type="datetime"
              value-format="timestamp" placeholder="Select date and time">
            </el-date-picker>
            <div v-if="item.elementType==5" class="select">
              <input class="text" type="text" placeholder="please select" v-model="item.models" disabled="disabled">
              <el-dropdown trigger="click" @command="handleCommand">
                <div class="selects arrow-down">
                  <i class="el-icon-arrow-down el-icon--right"></i>
                </div>
                <el-dropdown-menu slot="dropdown" style="height:210px;width:460px;
                overflow-y: scroll;
                  scrollbar-width: none;
                  -ms-overflow-style: none;">
                  <el-dropdown-item v-for="(i,index) in JSON.parse(item.content)" :key="index" :command="i+'/'+idx">
                    {{i}}
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
        ...
        <p  class="tag">Fields with <span style="color:#ff5959">*</span> are required</p>
  ...
  export default {
    ...
    methods: {
      ...
      for (let i=0; i < this.question.length;i++){
        ...
        this.$api.saveleavemsg(obj).then(
            res=>{
              if(res.data==1){
                this.success=true
              }
    ...
    created() {
      ...
      this.$api.getcustomfield(data).then(
        res => {
