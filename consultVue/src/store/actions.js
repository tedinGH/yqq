...
export const getallowmobile = ({
  commit
}, payload) => {
  return new Promise((resolve, reject) => {
    actionApi.getallowmobile(payload, data => {
      // commit("SET_ISALLOWMOBILE", data);
      resolve(data);
    });
  });
};
...
export const getReadNum = ({
  ...
  actionApi.getReadNum(payload, data => {
    if (data) {
      if (sId == store.state.customerServiceId && store.state.currentSession.fromType == 1) {
