  ...
  >
    ...
    >
      <div class="before-friend display-flex">
        <img class="before-friend-avatar" :src="global.fileDownUrl + 'compress/'+ friendInfo.headImg" v-if="friendInfo.headImg" v-headError/>
        <img class="before-friend-avatar" src="../assets/images/error-head.png" v-if="!friendInfo.headImg">
      ...
      >
       <div class="video-btn-list">
        ...
        <div class="video-btn-list">
          ...
          <div class="video-btn-item">
            <i class="btn notmute" :class="{ mute: mute }" @click="muteAudio"></i>
...
export default {
  ...
  watch: {
    ...
    andioVideoInfo() {
      if (this.andioVideoInfo.status == 0 || (this.andioVideoInfo.obj && this.andioVideoInfo.obj.status == 0)) {
        this.otherLeave();
      } else if (this.andioVideoInfo.status == 1 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 1)) {
        this.$store.commit(
          "SET_TOAST_TEXT",
          this.$t("msg.msgPanel.noResponse")
        );
      } else if (this.andioVideoInfo.status == 2 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 2)) {
        this.error.show = false;
        this.$store.commit(
          "SET_TOAST_TEXT",
      ...
      } else if (this.andioVideoInfo.status == 3 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 3)) {
        if (this.andioVideoInfo.resource == 1 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.resource == 1)) {
          if(this.andioVideoInfo.fromId!=undefined || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.fromId!=undefined)){
              //app接听
              let box = document.getElementById('gmsg-msgbox')
              if(box) return
              this.endAudio("audio_dail");
            ...
            this.$store.commit("UPDATE_VIDEO_INFO", { 
              status: 7 ,
              vType:this.andioVideoInfo.vType
              });
          }
      } else if (this.andioVideoInfo.status == 4 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 4)) {
        this.$store.commit(
          "SET_TOAST_TEXT",
          this.$t("msg.msgPanel.hangUp") + this.$t("msg.call.call")
        );
      } else if (this.andioVideoInfo.status == 5 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 5)) {
        this.$store.commit("UPDATE_VIDEO_INFO", { status: 7 });
        this.$store.commit(
          "SET_TOAST_TEXT",
          this.$t("msg.msgPanel.refuse") + this.$t("msg.call.call")
        );
      } else if (this.andioVideoInfo.status == 6 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 6)) {
      ...
      } else if (this.andioVideoInfo.status == 7 || (this.andioVideoInfo.obj &&this.andioVideoInfo.obj.status == 7)) {
        //
      } else if (this.andioVideoInfo.status == 8 ||(this.andioVideoInfo.obj && this.andioVideoInfo.obj.status == 8)) {
        this.getSocket("3");
        this.$store.commit("UPDATE_VIDEO_INFO", { status: 7 });
        this.resetData();
        rtc.leave();
  ...
  methods: {
    ...
    getArg(resolve) {
      ...
      this.$http.startCommunication(obj).then(
        data => {
          let errorUserId=data.errorUserId;
          if(errorUserId){
            if(errorUserId.length>0){
              for(let key in errorUserId){
                if(this.video.receiverId==errorUserId[key]){
                  this.$store.commit(
                    "SET_TOAST_TEXT",
                    `${this.$t("msg.msgPanel.otherBusy")}`
                  );
                  this.cancel();
                  this.$store.commit("UPDATE_VIDEO_INFO", { status: 3 });
                  return ;
                }
              }
            }
        ...
        () => {
          this.resetData();
          this.$store.commit("UPDATE_VIDEO_INFO", { status: 7 });
          this.$store.commit("UPDATE_AUDIO_STATE", false);
        }
      );
    },
    getSocket(s) {
    ...
    volumeClick($event) {
      ...
      let progress = event.clientY - this.videoBox.top - 425;
      this.volume.top = progress;
      if (progress < 0) {
        medio.volume = 0;
