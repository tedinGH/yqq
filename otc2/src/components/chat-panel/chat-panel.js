...
import MsgListItem from '@/components/char-panel-components/MsgListItem.vue';
import SystemMsgListItem from '@/components/char-panel-components/SystemMsgListItem.vue';
...
export default {
  ...
  data() {
    return {
      ...
      forwordSelect: false,
      exceed: false,
      MsgListItemComponent: MsgListItem,
      SystemMsgListItemComponent: SystemMsgListItem,
    };
  },
  components: {
    ChatSetting,
  ...
  watch: {
    ...
    chatHistory(newValue, oldValue) {
      ...
      // debugger
      console.log(this.getVirtualListOffset(), this.getVirtualListScrollHeight())
      if (newValue.length - oldValue.length < 5 && this.getVirtualListOffset() < 30) {
        this.$nextTick(() => {
          this.setVirtualListToBottom()
        });
      } else {
  ...
  provide() {
    return {
      ...
      voicePlay: this.voicePlay,
      showmemberProfile: this.showmemberProfile,
      historySelect: this.historySelect,
      openRecord: this.openRecord,
      goFocus: this.goFocus,
      groupInvite: this.groupInvite,
      loadOriginalImg: this.loadOriginalImg,
  ...
  methods: {
    ...
    showMsg(record) {
      ...
      this.$nextTick(() => {
        const sids = this.getSids(data, 'mId');
        console.log(sids)
        if (sids.length > 0) {
          const vsl = this.$refs.list
          const offset = sids.reduce((previousValue, currentSid) => {
            ...
            console.log(previousSize, previousValue, currentSid)
            return previousSize + vsl.getSize(currentSid);
          })
          // console.log(offset);
          this.setVirtualListToOffset(offset - 500);
        }
        this.scrollFlag = 0;
        if (dataLength < 30) {
          this.scrollFlag = 2;
                ...
                setTimeout(() => {
                  ...
                  if (list && list.length) {
                    ...
                    if (id >= minId && id <= maxId) {
                      ...
                      if (hasId) {
                        this.setVirtualListScrollToIndex(id);
                        this.atId = id;
                        setTimeout(() => {
                          this.atId = "";
                        }, 1000);
    ...
    setVirtualListToBottom() {
      if (this.$refs.list) {
        ...
        console.log(this.$refs.list.getScrollSize())
      }
    },
    ...
    setVirtualListScrollToIndex (index) {
      if (this.$refs.list) {
        this.$refs.list.scrollToIndex(index)
      }
    },
    getVirtualListOffset() {
      if (this.$refs.list) {
        return this.$refs.list.getOffset();
      }
      return 0;
    },
    getVirtualListScrollHeight () {
      if (this.$refs.list) {
        return this.$refs.list.getScrollSize();
      }
      return 0;
    },
    // 虚拟滚动容器end
