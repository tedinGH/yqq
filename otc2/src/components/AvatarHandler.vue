  ...
  <transition name="pop">
    <div class="my-popup" @click.stop="">
      <div class="my-popup-content avatar-handler">
        ...
        <div class="upload-avatar avatar2" v-if="fileData.img">
        ...
        <div class="upload-avatar " v-else>
          <label for="avatar" class="upload-btn"></label>
          <input
            id="avatar"
            type="file"
          ...
          />
          <p style="color:#45B4AA">{{ $t("msg.user.upload")}}</p>
...
export default {
  ...
  methods: {
    ...
    uploadAvatar() {
      ...
      let that=this;
      reader.readAsArrayBuffer(that.fileData.file);
      reader.onload = function(){
        var data = this.result;
      
          let a={fileName:that.fileData.file.name};
          let dataParams=JSON.stringify(a)
          let myArray=msgUtil.msgEntityToBufferV1(that.$store.getters.token,"/uploadV2/user/avatar.htm",dataParams,data);
      ...
      // this.fileData.file.arrayBuffer().then(data=>{
      //     let a={fileName:this.fileData.file.name};
      //     let dataParams=JSON.stringify(a)
      //     let myArray=msgUtil.msgEntityToBufferV1(this.$store.getters.token,"/uploadV2/user/avatar.htm",dataParams,data);
      //     var xml = new XMLHttpRequest();
      //     xml.open("post", base.imfileUrl+"/apiV1.htm", true);
      //     xml.send(myArray);
      //     xml.onreadystatechange = () => {
              ...
              setTimeout(() => {
                ...
                if (data.code == 0) {
                  ...
                  setTimeout(() => {
                    ...
                    that.$emit("changeAvatar", data.data);
                    that.clearFile();
                  }, 1600);
                } else {
                  that.showFail = true;
...
.avatar-handler {
  ...
  .upload-avatar {
    ...
    .upload-btn {
      ...
      width: 60px;
      height: 60px;
      background:#45B4AA url(../assets/images/add2.png) no-repeat;
      background-position: 50% 50%;
      border: 1px solid #A2D9D4;
      border-radius: 50%;
      margin: 101px auto 22px;
      cursor: pointer;
    }
  }
  .operation {
