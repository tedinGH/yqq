  ...
  <transition name="scale-x">
    <div class="my-popup search-record" @click="hide" v-if="isShow">
      <div class="my-popup-content" @click.stop="hideRight">
        ...
        <div class="content" v-show=" !resultList.length">
          <span class="dataNone1"></span>
          <p style="color:#5F717F">{{$t('msg.friendList.searchKey')}}</p>
        </div>
        <div class="content" v-show="searchKey!='' && (resultList && resultList.length)">
          <div class="content-left">
            <ul v-scrollBar>
              <li :class="{active: item.paramId == paramId}" @click="openRecord(item)" v-for="(item,idx) in resultList"
                :key="idx">
                <div class="item-left">
                  <img v-if=" item.img" v-headError :src="global.fileDownUrl + 'compress/' +item.img" class="avatar">
                  <img class="avatar" src="../../assets/images/error-head.png"  v-if="!item.img&&item.fromType==0" >
                  <img class="avatar" src="../../assets/images/error-head2.png"  v-if="!item.img&&item.fromType==1" >
                </div>
          ...
          <div class="content-right" v-show="recordList && recordList.length">
            <div class="title">
              <span>{{recordList.length}}
                {{ $t("msg.search.and") }}“{{searchKey}}”{{ $t("msg.search.andRecordNum") }}</span>
            ...
            <ul v-scrollBar>
              <li v-for="(item, index) in recordList" :key="index"
                :class="{ 'more-select': showMore,'active': (item.checked && showMore) }" @click="goMsg(item.mId)"
                style="cursor: pointer;">
                <span class="select" :class="{ active: item.checked }"></span>
                <span class="selectArea" v-show="showMore" @click.stop="historySelect(item)"></span>
                <section :data-index="`{%type%:%record%,%mId%:${item.mId},%banAt%:true}`">
                  <div class="item-left">
                    ...
                    <!-- <img class="avatar" src="../../assets/images/error-head2.png"  v-if="record.fromType==1&&!item.chatInfo.headImg"> -->
                    ...
                    <img class="avatar" src="../../assets/images/error-head.png"  v-if="record.fromType==0&&!record.img">
  ...
  .my-popup-content {
    ...
    .content{
      ...
      .dataNone1{
        ...
        width:140px;
        margin:107px 290px 0 290px;
        background-image: url(../../assets/images/search/datanone.png);
        background-size: 100% 100%;
      }
      .content-left{
        float:left;
