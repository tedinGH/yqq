...
export default {
  ...
  onMessage(msg) {
    ...
    try {
      ...
      switch (body.bodyType + "") {
        ...
        case "16":
          if (
            userId == store.state.currentSession.paramId &&
            fromType == store.state.currentSession.fromType
          ) {
            let body = JSON.parse(msgObj.msgContent);
            if (body.bodyFrom == store.state.userInfo.userId) return;
            let content = msgFormatTemplate.getPlaintextMsg(
              msgObj.fromType,
              msgObj.userId,
              body
            );
            ...
            store.commit("SET_IS_TYPING", {
              status: true,
              type: content.msgContent.type,
            });
          }
          break;
        ...
        case "19": //接收群指令  刷新群成员信息
          ...
          if (bodyContent.type == 12) {
            ...
            if (sCache) {
              ...
              store.commit(
                "UPDATE_SESSION",
                JSON.parse(JSON.stringify(sCache))
              );
            }
            if (
              store.getters.currentSession.fromType == 1 &&
              body.bodyFrom == store.getters.currentSession.paramId
            ) {
              ...
              if (bodyContent.type == 13) {
                if (bodyContent.auditResult == 1) {
                  ...
                  if (Number(inviter.userId) == store.getters.userId) {
                    var sessionObj =
                      store.state.session.record[body.bodyFrom + "-1"];
          ...
          //收到邀请通过进群，更新邀请信息
          if (bodyContent.type == 22) {
            if (bodyContent.operatorId == store.getters.userId) {
              localStore.updateInviterCode(body.bodyFrom, bodyContent);
              store.commit("SET_GROUP_INVITE", bodyContent);
            }
          }
          //删除群公告
        ...
        case "20": //消息发送 成功回执
          ...
          if (
            store.state.currentSession.paramId == userId &&
            store.state.currentSession.fromType == type
          ) {
            let history1 = store.getters.message[mId];
            if (history1) {
              store.commit("DELETE_MESSAGE", history1.mId);
              history1.mId = nId;
              history1.lastReadId = nId;
              history1.loading = 1;
              history1.readNum = 0;
              history1.bodyTime = body.bodyTime;

              //文件类型更新
              if (history1.bodyType == 18) history1.msgContent.progress = 100;
              // //发送成功提示音
              // Msg.sendNotice();
              store.commit("UPDATE_MESSAGE", history1);
            }
          }

          //可能遗留有串ID，不刷UI的坑
          let history = store.state.sendingMessage[mId];
          if (history) {
            history.mId = nId;
            ...
            delete store.state.sendingMessage[mId];
          }
        ...
        case "25": //添加好友申请
          if (body.bodyContent == "1") {
            ...
            // Msg.updateHistory(msgObj);
            let cacheSession =
              store.state.session.record[userId + "-" + fromType];
            if (cacheSession) {
            ...
            if (body.bodyFrom == store.getters.currentSession.paramId) {
              store.commit(
                "UPDATE_CURRENT_SESSION",
                JSON.parse(JSON.stringify(cacheSession))
              );
            }
        ...
        case "31": //消息被拒收
          if (body.bodyFrom != store.getters.userId) {
            if (
              userId == store.state.currentSession.paramId &&
              store.state.currentSession.fromType == fromType
            ) {
              ...
              if (msg.type == 3) {
                ...
                store.commit(
                  "UPDATE_CURRENT_SESSION",
                  JSON.parse(JSON.stringify(store.state.currentSession))
                );
              }
        ...
        case "34": //置顶消息指令
          ...
          let session =
            store.state.session.record[body.bodyFrom + "-" + body.bodyFromType];
          if (content.type == 0) {
        ...
        case "36": //音视频操作
          var content = JSON.parse(body.bodyContent);
          if(content.callSource && content.callSource!=2) return
          if (content.state == "1") {
            if (store.state.isOnlineAudio) {
              let videoInfo = store.getters.andioVideoInfo;
              if (
                videoInfo.fromId == content.fromId &&
                videoInfo.type == content.type &&
                videoInfo.vType != content.vType
              ) {
              } else {
                break;
              }
            } else {
              store.commit("UPDATE_AUDIO_STATE", true);
            }
            if (content.type == "3") {
              if (store.getters.andioVideoInfo.fromId != content.fromId) {
            ...
            store.commit("UPDATE_AUDIO_STATE", false);
          }
        ...
        case "38": //音视频消息
          var content = JSON.parse(body.bodyContent);
          if (content.resource) {
            let obj = store.state.videoInfo;
            obj.status = 3;
            obj.resource = content.resource;
        ...
        case "42": //删除消息指令
          ...
          if (content.type != -1) {
            ...
            //清空列表
            let sessionCache =
              store.state.session.record[
                body.bodyFrom + "-" + body.bodyFromType
              ];
  ...
  retrieveMsg(userId, type, msgId) {
    ...
    if (cacheSession) {
      if(!cacheSession.msgInitialId) cacheSession.msgInitialId = 0
      ...
      if (differNum <= 1) {
        ...
        //消息丢失
        // 计算断层
        let oldId = cacheSession.msgInitialId;
        if (differNum > 200) {
          oldId = msgId - 200;
        }
        msgManager.getFaultMsg(Number(oldId) + 1 + "-" + msgId, userId, type);
      }
    }
  },
};
