...
export const Util = {
  ...
  getImgUrl(file, type) {
    return new Promise((resolve, reject) => {
      ...
      let dataParams = null;
      if (type == 1) {
        apiName = "/uploadV2/picture.htm";
      } else if (type == 2) {
        apiName = "/uploadV2/picture.htm";
        a.privateFile = true;
      } else {
        a.fileName = new Date().getTime() +  Math.floor(Math.random()*(1 - 100) + 1) + ".png"
        file = this.dataURLtoBlob(decodeURIComponent(file));
        // a.base64=decodeURIComponent(file);
        // apiName = "/uploadV2/picture64.htm";
        apiName = "/uploadV2/picture.htm";
      }
      dataParams = JSON.stringify(a);
      var reader = new FileReader();
      reader.readAsArrayBuffer(file);
      reader.onload = function () {
        var data = this.result;
        try {
          let myArray = msgUtil.msgEntityToBufferV1(
            user.token,
            apiName,
            dataParams,
            data
          ...
          xml.onreadystatechange = () => {
            if (xml.readyState == 4) {
              if (xml.status == 200) {
                ...
                let toastMsg =
                  lang.common.image +
                  lang.common.uploadfail +
                  "," +
                  lang.common.uploadAgain;
                reject(toastMsg);
              }
            }
          };
      ...
      //     try {
      //       let myArray=msgUtil.msgEntityToBufferV1(user.token,apiName,dataParams,data);
      //       let xml = new XMLHttpRequest();
      //       xml.open("post", base.imfileUrl+"/apiV1.htm", true);
      //       xml.send(myArray);
      //       xml.onreadystatechange = () => {
      ...
      // }else{
      //   try {
      //     let myArray=msgUtil.msgEntityToBufferV1(user.token,apiName,dataParams);
      //     let xml = new XMLHttpRequest();
      //     xml.open("post", base.imfileUrl+"/apiV1.htm", true);
      //     xml.send(myArray);
      //     xml.onreadystatechange = () => {
  ...
  getImgUrl1(file, type) {
    return new Promise((resolve, reject) => {
      ...
      if (type == 1) {
        ...
        newform.append("base64", decodeURIComponent(file));
  ...
  uploadFile(fileArr, fileData, count) {
    return new Promise((resolve, reject) => {
      var user = JSON.parse(
        Crypto.decryptByDES(sessionStorage.getItem("eUcShEaRt"))
      );

      var reader = new FileReader();
      let that=this;
      reader.readAsArrayBuffer(fileArr[count].file);
      reader.onload = function () {
        var data = this.result;
        try {
          let a = {
            fileName: fileArr[count].file.name,
            fileIndex: that.cutSize() * count,
            fileId: fileData.fileId,
          };
          let dataParams = JSON.stringify(a);
          let myArray = msgUtil.msgEntityToBufferV1(
            user.token,
            "/fileShare/fileUploadV2.htm",
            dataParams,
            data
          );
      ...
      //   fileArr[count].file.arrayBuffer().then(data=>{
      //     try {
      //       let a={fileName:fileArr[count].file.name,fileIndex:this.cutSize()*count,fileId:fileData.fileId};
      //       let dataParams=JSON.stringify(a)
      //       let myArray=msgUtil.msgEntityToBufferV1(user.token,"/fileShare/fileUploadV2.htm",dataParams,data);
  ...
  uploadFile1(fileArr, fileData, count) {
    return new Promise((resolve, reject) => {
      var user = JSON.parse(
        Crypto.decryptByDES(sessionStorage.getItem("eUcShEaRt"))
      );
