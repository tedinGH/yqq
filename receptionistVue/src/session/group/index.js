    ...
    return new Promise(resolve => {
      groupDB
        ...
        .then(data => {
          ...
          //获取群信息接口
          actionApi.getGroupSetting({ groupId: groupId }).then(data => {
    ...
    return new Promise(resolve => {
      groupDB.readGroupMemberInfo(groupId, userId).then(data => {
        if (data) {
          ...
          actionApi.getGroupUser({ groupId: groupId, userId: userId }).then(data => {
  ...
  getMsg(sessionId, maxMsgId, firstMsgId, pageSize = 20) {
    ...
    return new Promise(resolve => {
      ...
      groupDB.readGroupMsgPage(sessionId, maxMsgId, pageSize).then(data => {
        let record = { firstMsgId: firstMsgId, paramId: sessionId, fromType: this.sessionType };
        if (data.length == 0) {
          //获取接口
          let obj = {
