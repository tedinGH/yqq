...
export const Util = {
  ...
  formatTime(value) {
    if (+value > 3600) {
      let h = Math.floor(value / 3600),
        ...
        s = value % 60;
      return this.doubleNumber(h) + ":" + this.doubleNumber(m) + ":" + this.doubleNumber(s);
    } else if (+value > 60) {
  ...
  insert(field, value, selectPastedContent) {
    var sel;
    var range;
    field.focus();
    if (window.getSelection) {
  ...
  getImgUrl(file) {
    return new Promise((resolve, reject) => {
      ...
      let url = urls.UPLOAD_FILE;
      xml.open("post", url, true);
      xml.setRequestHeader("Accept-Language", store.getters.lang || "en_US");
      xml.setRequestHeader("deviceType", 2);
      xml.setRequestHeader("version", "1.0.0.1");
      xml.setRequestHeader("Network", "wifi");
      xml.setRequestHeader("token", store.getters.token);
      xml.send(newform);
    ...
    return new Promise((resolve, reject) => {
      ...
      browserMD5File(file, (err, md5) => {
        ...
        let postData = {
          ...
          fileSize: file.size,
          fileMd5: md5,
          fileBlock: that.cutSize()
        };

        actionApi.ininFileChannelUploadFile(postData).then(
          data => {
            if (data.code == 0) {
              var fileData = {
                ...data.data,
              ...
              //分割切片上传
              console.log(fileArr)
              that.uploadFile(fileArr, fileData, 0).then(uploadFileData => {
                console.log(data);
                if (data && data.code == 0) {
                  resolve(uploadFileData);
                  return;
                } else {
                  reject();
                  return;
  ...
  uploadFile(fileArr, fileData, count) {
    return new Promise((resolve, reject) => {
      ...
      formData.append("fileIndex", that.cutSize() * count);
      ...
      xml.open("post", urls.UPLOAD_FILE_V2, true);
      xml.setRequestHeader("Accept-Language", store.getters.lang || "en_US");
      xml.setRequestHeader("deviceType", 2);
      xml.setRequestHeader("version", "1.0.0.1");
      xml.setRequestHeader("Network", "wifi");
        ...
        if (xml.status == 200 && uploadReady) {
          ...
          setTimeout(() => {
            ...
            if (count != fileArr.length) {
              // 断点
              that.uploadFile(fileArr, fileData, count);
            } else {
              console.log(obj);
              resolve(fileData);
            }
  ...
  sortByLetter(payload, isGroup) {
    ...
    for (let item in payload) {
      ...
      for (let re of result) {
        let name = payload[item].notes || payload[item].nickName || payload[item].userName;
...
// 解构会话列表公共字段
export const sessionItemPool = obj => {
  const {
    source,
    visitorId,
    id,
  ...
  let { newMsg = "" } = obj;
  newMsg = Util.replaceText(newMsg || "");
  return {
    id,
    source,
    sessionId,
    newMsg,
    type: msgType || newMsgType || type,
    name: ipSource,
    chatId: visitorId,
    isActivity,
    online: 1,
    avatar: undefined,
    time: newMsgTime || updateTime
  };
};

// 解构消息公共字段
export const msgItemPool = obj => {
  const { isSender = true, type = null, msgType = null, chatType, fromName, fromId, toId, createTime, msg, msgId } = obj;
  return {
    isSender,
    chatType,
    type,
...
export function repalceToRed(content, text) {
  var reger = new RegExp(content, "g");
  return text.replace(reger, "<span style='color: red;'>$&</span>");
};

export function exportHtml(name, data) {
