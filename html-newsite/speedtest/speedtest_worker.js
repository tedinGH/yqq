...
var settings = {
	...
	url_dl: "https://download.bcirclepay.com:8443/imfileweb/speed/garbage.htm", // path to a large file or garbage.php, used for download test. must be relative to this js file
	url_ul: "https://download.bcirclepay.com:8443/imfileweb/speed/empty.htm", // path to an empty file, used for upload test. must be relative to this js file
	url_ping: "https://download.bcirclepay.com:8443/imfileweb/speed/empty.htm", // path to an empty file, used for ping test. must be relative to this js file
	url_getIp: "https://fi.openspeed.org/getIP.php", // path to getIP.php relative to this js file, or a similar thing that outputs the client's ip
	url_dl_s3: "https://d12kubtoru2eej.cloudfront.net/original/2020/05/13/Uv3zkyljMzBKHPA5Jwk.png",
	...
	xhr_multistreamDelay: 600, //how much concurrent requests should be delayed
	...
	url_telemetry: "https://download.bcirclepay.com:8443/imfileweb/speed/telemetry.htm", // path to the script that adds telemetry data to the database
...
this.addEventListener("message", function(e) {
	...
	if (params[0] === "status") {
		...
		postMessage(
			JSON.stringify({
				...
				clientIp: clientIp,
				clientIpOnly: clientIpOnly, // IP trim local
	...
	if (params[0] === "setS3") {
		setS3Download(params[1]);
	}else if(params[0] === "param"){
		paramUrl=JSON.parse(params[1]);
...
function getIp(done) {
	...
	xhr.onload = function() {
		...
		try {
			...
			clientIpOnly = ispInfo.ip;
			rawIspInfo = data.rawIspInfo;
		} catch (e) {
...
var dlCalledS3 = false; // used to prevent multiple accidental calls to dlTest
function dlS3Test(done) {
	debugger;
	tverb("dlS3Test");
	if (dlCalledS3) return;
	else dlCalledS3 = true; // dlTest already called?
	var startT = new Date().getTime();
	var x = new XMLHttpRequest();
	x.onload = function() {
		var size = 13958093;
		var endTime = new Date().getTime();
		var speed = parseInt(size / (endTime - startT));
		dlStatusS3 = (speed / 1024).toFixed(2);
		done();
	}
	x.open("PUT", settings.url_dl_s3 + "?timeStamp=" + startT, true); 
	x.send();
}
...
function sendTelemetry(done) {
	if(dlStatus!=""&&ulStatus!=""&&pingStatus!=""&&jitterStatus!=""&&dlStatusS3!=""){}else{return ;}
	...
	try {
		...
		// fd.append("s3Dl", dlStatusS3);

		let a=clientIp.split(' - ');

		fd.append("ispinfo", JSON.stringify(telemetryIspInfo));
		fd.append("downloadSpeed", dlStatus);
		fd.append("uploadSpeed", ulStatus);
		fd.append("ping", pingStatus);
		fd.append("jitter", jitterStatus);
		fd.append("log", settings.telemetry_level > 1 ? log : "");
		fd.append("area", a[1]);
		fd.append("s3DownloadSpeed", dlStatusS3);

		fd.append("device", decodeURIComponent(paramUrl.device));
		fd.append("network", paramUrl.network);
		fd.append("appVersion", paramUrl.appVersion);
		fd.append("cdnChannel", dlStatusS3);
		fd.append("userId", paramUrl.userId);
		fd.append("ipAddr", a[0]);
...
function setS3Download(speed){
	dlStatusS3=speed;
	sendTelemetry(function() {testState = 4;});
}